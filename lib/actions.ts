...
export async function getAllUserCurrentTournaments() {
  ...
  if (error) {
    console.error("MAIN ERROR (getAllUserCurrentTournaments):", error);
  }
  if (joinedError) {
    console.error("JOIN ERROR (getAllUserCurrentTournaments):", joinedError);
  }

  const joinedTournaments = joinedError ? [] : (joined || []).flatMap((item) => item.tournaments || []);
  const uniqueTournaments = new Map<string, Tournament>();

  if (!error && ownTournaments) {
...
export async function getAllUserOwnedPublicTournaments(userId: string) {
  ...
  if (error) {
    console.error("ERROR (getAllUserOwnedPublicTournaments):", error);
...
export async function getAllUserMatchResults(userId: string) {
  ...
  if (error) {
    console.error("ERROR (getAllUserMatchResults):", error);
    return { error: 'Failed to fetch match results' };
  }

  return { matchesWithUsernames: matchesWithUsernames || [] };
}
...
export async function getPublicUserData(
  ...
  if (error) {
    console.error("Supabase Error in getPublicUserData:", JSON.stringify({
      code: error.code,
  ...
  return { data: data as PublicUser | null, error: null };
}
